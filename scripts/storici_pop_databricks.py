...
DBFS_BASE_FOLDER = "/FileStore/download_storici"
...
def crea_tabelle(conn):
    with conn.cursor() as cur:
        ...
        cur.execute(f"""CREATE TABLE IF NOT EXISTS {LOG_TABLE_NAME} (
                filename TEXT PRIMARY KEY, processed_at TIMESTAMP DEFAULT NOW());""")
        conn.commit()
...
def main():
    ...
    try:
        ...
        print(f"Trovati {len(processed_files)} file già processati nel database.")

        # --- 2. Trova le cartelle dei download su DBFS per la data specificata ---
        print(f"\nCerco cartelle di download in DBFS per la data: {DATA_ELABORAZIONE}...")
        all_content = list_dbfs_content(DBFS_BASE_FOLDER)

        folders_to_process = [
            item["path"] for item in all_content
            if item["is_dir"] and os.path.basename(item["path"]).startswith(DATA_ELABORAZIONE)
        ]

        if not folders_to_process:
            print(f"Nessuna cartella di download trovata per la data {DATA_ELABORAZIONE}. Uscita.")
            return

        print(f"Trovate {len(folders_to_process)} cartelle da elaborare: {folders_to_process}")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for daily_folder_path in folders_to_process:
                print(f"\n--- Inizio elaborazione cartella DBFS: {daily_folder_path} ---")

                # --- 5. Itera su ogni strada configurata (A90, SS51, etc.) ---
                for strada_nome, config in STRADE_CONFIG.items():
                    table_name = config['table_name']
                    punto_tratto_map = config['punto_tratto_map']
                    dbfs_road_path = f"{daily_folder_path}/{strada_nome}"

                    print(f"  Verifico la strada '{strada_nome}' in '{dbfs_road_path}'...")
                    ...
                    if not json_files_in_dbfs:
                        print(f"    Nessun file JSON trovato per '{strada_nome}' in questa cartella.")
                        continue
                    ...
                    for file_info in json_files_in_dbfs:
                        ...
                        print(f"    Nuovo file: {filename}. Scarico in temporanea...")

                        # Scarica il file in locale nella cartella temporanea
                        local_filepath = os.path.join(temp_dir, filename)
                        if not download_dbfs_file(file_info["path"], local_filepath):
                            continue  # Salta il file se il download fallisce

                        # --- 6. Elabora il file temporaneo (logica originale) ---
                        ...
                        if not all([punto, tratto, timestamp_file]):
                            print(f"      [AVVISO] Metadati mancanti o tratto non mappato per {punto}. Salto.")
                            continue

                        # Carica, parsa e inserisce i dati dal file locale temporaneo
                        # ... (la logica di parsing è invariata) ...
                        with open(local_filepath, "r") as f:
                            data = json.load(f)

                        # (Il resto del codice di parsing e inserimento è identico all'originale)
                        time_list = data.get("data_1h", {}).get("time", [])
                        rows = []
                        for i, time_str in enumerate(time_list):
                            row = {"downloaded_at": timestamp_file.replace(microsecond=0), "tratto": tratto,
                                   "punto": punto,
                    ...
                    if nuovi_file_per_strada == 0:
                        print(f"    Nessun *nuovo* file da processare per {strada_nome} in questa cartella.")
        ...
        if conn: conn.close()
        print("\nElaborazione completata. La cartella temporanea è stata rimossa.")
